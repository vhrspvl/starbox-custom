...
def execute(filters=None):
    ...
    for emp in sorted(att_map):
        ...
        row = [emp, emp_det.employee_name, emp_det.branch, emp_det.department, emp_det.designation, emp_det.contractor, emp_det.employment_type,
               emp_det.company]
        ...
        for day in range(filters["total_days_in_month"]):
            ...
            status_map = {"Present": "P", "Absent": "A", "Half Day": "HD",
                          "On Leave": "L", "On Duty": "OD", "None": "", "Holiday": "<b>H</b>"}
...
def get_columns(filters):
    columns = [
        ...
        ":Link/Designation:120", _("Contractor") + ":Link/Contractor:120", _("Employment Type") +
...
def get_employee_details():
    ...
    for d in frappe.db.sql("""select name, employee_name, designation,contractor,department, branch, employment_type, company,
...
def get_holiday(holiday_list, month):
    ...
    for d in holiday_list:
        if d:
            holiday_map = holiday_map.setdefault(d, frappe.db.sql_list('''select day(holiday_date),description from `tabHoliday`
