...
def execute(filters=None):
    ...
    att_map, in_time_map, out_time_map = get_attendance_list(
        conditions, filters)
    ...
    for emp in sorted(att_map):
        ...
        row = [emp, emp_det.employee_name, emp_det.biometric_id, emp_det.department,
               emp_det.designation, emp_det.employment_type, emp_det.contractor]
...
def get_columns(filters):
    columns = [
        ...
        "::120", _("Employment Type") +
        ":Link/Employment Type:120", _("Contractor") +
        ":Link/Contractor:120"
    ]
...
def get_attendance_list(conditions, filters):
    attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status,in_time,out_time from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
    ...
    for d in attendance_list:
        ...
        in_time_map.setdefault(d.employee, frappe._dict()
                               ).setdefault(d.day_of_month, "")
        out_time_map.setdefault(d.employee, frappe._dict()
                                ).setdefault(d.day_of_month, "")
        att_map[d.employee][d.day_of_month] = d.status
        in_time_map[d.employee][d.day_of_month] = d.in_time
        out_time_map[d.employee][d.day_of_month] = d.out_time
    return att_map, in_time_map, out_time_map
...
def get_employee_details():
    ...
    for d in frappe.db.sql("""select name, employee_name, biometric_id,designation,employment_type, department, contractor,
