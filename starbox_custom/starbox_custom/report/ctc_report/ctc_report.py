...
def execute(filters=None):
	...
	columns, earning_types, ded_types, cont_types = get_columns(
		filters, salary_str)
	data = []
		...
		for ss in salary_str:
			for emp in emp_lst:
				if ss.employee == emp.name:
					row = [ss.name, ss.from_date, ss.to_date,
                                            ss.is_active, emp.name, emp.employee_name]
			tot_earn = 0
...
def get_data_for_eval(ss):
	...
	for d in ss_doc.employees:
		if d.employee == ss.employee:
			eval_data.update(frappe.get_doc("Salary Structure Employee", {
			                 "employee": d.employee}).as_dict())
...
def get_ss_cont_map(salary_str):
	...
	ss_cont_map = {}
	for d in ss_contri:
		ss_cont_map.setdefault(d.parent, frappe._dict()
		                       ).setdefault(d.salary_component, [])
...
def get_columns(filters, salary_str):
	if filters.get("without_salary_structure") <> 1:
		columns = [
			...
			_("To Date") + ":Date:80", _("Active") +
               			  "::40", _("Employee") + ":Link/Employee:80",
			_("Name") + "::150"
		]
